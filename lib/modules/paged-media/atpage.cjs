  ...
  (0, _createClass2["default"])(AtPage, [{
    ...
    */
    ...
    value: function getStartElement(content, breakToken) {
      ...
      var node = breakToken && (((_breakToken$overflow$ = breakToken.overflow[0]) === null || _breakToken$overflow$ === void 0 ? void 0 : _breakToken$overflow$.node) || breakToken.node);
    ...
    key: "afterPageLayout",
    value: function afterPageLayout(page, contents, breakToken, chunker) {
      var thisPage = chunker.pages[chunker.pages.length - 1];
      // If only footnotes were added, attribs should be like the previous page.
      var emptyBody = !thisPage.area.firstElementChild || !thisPage.area.firstElementChild.childElementCount || !thisPage.area.firstElementChild.firstElementChild.getBoundingClientRect().height;
      var emptyFootnotes = !thisPage.footnotesArea.firstElementChild.childElementCount || !thisPage.footnotesArea.firstElementChild.firstElementChild.getBoundingClientRect().height;
      if (emptyBody && !emptyFootnotes && chunker.pages.length > 1) {
        // Start element for the previous page.
        var prevBreakToken = chunker.pages[chunker.pages.length - 2].startToken;
        var start = this.getStartElement(contents, prevBreakToken);
        if (start) {
