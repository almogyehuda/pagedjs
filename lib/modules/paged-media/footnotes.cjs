...
var Footnotes = /*#__PURE__*/function (_Handler) {
  ...
  (0, _createClass2["default"])(Footnotes, [{
    ...
    key: "recalcFootnotesHeight",
    value: function recalcFootnotesHeight(node, noteContent, pageArea, noteCall, needsNoteCall) {
      // Remove empty class
    ...
    value: function moveFootnote(node, pageArea, needsNoteCall) {
      ...
      if (needsNoteCall) {
        if (node.parentElement) {
          ...
          var ref = node.dataset['ref'];
          noteCall = pageArea.querySelector("[data-ref=\"".concat(ref, "\"]"));
        }
      }
    ...
    value: function afterPageLayout(pageElement, page, breakToken, chunker) {
      ...
      if (overflow) {
        var _overflow$ = overflow[0],
          startContainer = _overflow$.startContainer,
          startOffset = _overflow$.startOffset;
        var extracted,
          parentElement = startContainer.parentElement;
        var footnoteContainer = (0, _dom.isText)(startContainer) ? startContainer.parentElement.closest('[data-footnote-marker]') : startContainer.closest('[data-footnote-marker]');
        var isEntireNote = footnoteContainer && (footnoteContainer == startContainer || footnoteContainer.childNodes[0] == startContainer && !startOffset);
        if (isEntireNote) {
        ...
        if (!breakToken) {
          ...
          var _breakToken$overflow$;
          var breakBefore, previousBreakAfter;
          var firstOverflowNode = (_breakToken$overflow$ = breakToken.overflow[0]) === null || _breakToken$overflow$ === void 0 ? void 0 : _breakToken$overflow$.node;
          if (firstOverflowNode && typeof firstOverflowNode.dataset !== "undefined" && typeof firstOverflowNode.dataset.previousBreakAfter !== "undefined") {
            previousBreakAfter = firstOverflowNode.dataset.previousBreakAfter;
          }
          if (firstOverflowNode && typeof firstOverflowNode.dataset !== "undefined" && typeof firstOverflowNode.dataset.breakBefore !== "undefined") {
    ...
    key: "afterOverflowAdded",
    value: function afterOverflowAdded(rendered) {
      var _this3 = this;
      var notes = rendered.querySelectorAll("[data-note='footnote']");
      if (notes && notes.length) {
        this.findVisibleFootnotes(notes, rendered);
      }
      var area = rendered.closest(".pagedjs_area");
      var noteContent = area.querySelector(".pagedjs_footnote_content");
      var notesInnerContent = area.querySelector(".pagedjs_footnote_inner_content");
      if (this.overflow.length) {
        this.overflow.forEach(function (item) {
          notesInnerContent.appendChild(item);
          var call = rendered.querySelector("[data-ref=\"".concat(item.dataset['ref'], "\"]"));
          _this3.recalcFootnotesHeight(item, noteContent, area, call, false);
        });
        this.overflow = [];
      }
    }
